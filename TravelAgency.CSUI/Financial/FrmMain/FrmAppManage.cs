...
{
    public partial class FrmAppManage : Form
    {
        //private readonly TravelAgency.BLL.ClientCharge _bllClentCharge = new TravelAgency.BLL.ClientCharge();
        private readonly TravelAgency.BLL.AppAll _bllAppAll = new TravelAgency.BLL.AppAll();
        ...
        public FrmAppManage(bool b4Add = false)
        {
        ...
        private List<Model.AppAll> DgvDataSourceToList()
        {
            return dataGridView1.DataSource as List<Model.AppAll>;
        }
        ...
        /// </summary>
        /// <returns></returns>
        private List<Model.AppAll> GetSelectedModelList()
        {
            var visaList = dataGridView1.DataSource as List<Model.AppAll>;
            List<Model.AppAll> res = new List<AppAll>();
            for (int i = dataGridView1.SelectedRows.Count - 1; i >= 0; i--)
                res.Add(DgvDataSourceToList()[dataGridView1.SelectedRows[i].Index]);
            return res.Count > 0 ? res : null;
        }

        private List<Model.QZApplication> GetQzListByAppAll(Model.AppAll appall)
        {
            var list = _bllQzApplication.GetModelList(" App_id = '" + appall.App_id + "' ");
            return list;
        } 
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBoxEx.Show("确认删除" + count + "条记录?",
                ...
                return;
            int n = 0;
            var visaList = GetSelectedModelList();
            for (int i = 0; i != visaList.Count; ++i)
            {
                if (!_bllAppAll.Delete(visaList[i].temp))
        ...
        private void 查看明细ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            var qzlist = GetQzListByAppAll(list[0]);
            FrmQZApplication frm = new FrmQZApplication(qzlist,LoadDataToDataGridView,_curPage);
            frm.ShowDialog();
